...
> 保留为内部 Beta 历史记录；v1 的公开边界以[兼容性契约](compatibility.md)为准。

## 当前判断

v0.5–v0.9 的内部阶段已经收敛为 v1.0 实现。正式 tag 构建仍必须通过三平台安装、恢复、
Provider、MCP、归档和长期运行清单；手动 workflow 只生成内部验证 artifact。
...
| v1.0 | 稳定的 Agent-native 本地音频知识库 | 实现完成，发布验收待执行 | 兼容、迁移、隐私、质量评测和回滚形成公开承诺 |
...
## v1.0：稳定的 Agent-native 本地音频知识库（实现完成；发布验收待执行）
...
平台、弃用策略、已知限制与回滚步骤见[兼容性契约](compatibility.md)。自动化门槛由测试与
匿名评测覆盖，目标系统的安装包、休眠恢复和长期运行仍须按[发布清单](release-checklist.md)
留存验收记录后再创建公开 tag。
...
1. 在 Linux、Windows 和 macOS 目标系统完成 v1.0 Tauri、browser-lite、MCP 与 Whisper
   companion 的 clean-install、长期运行、休眠恢复、端口冲突和正常退出验收。
2. 留存归档 dry-run/导入、数据库快照回滚、诊断包脱敏和受控 Agent 审批的独立安全复核
   记录。
3. 持续运行固定检索、引用、勘误和 Tag 评测，记录质量与资源回归；候选增强项不应抢占
   v1.0 发布稳定性门槛。
