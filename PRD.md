...
| v0.5 | 完成 Beta 发布加固；增加 playlist 管理、媒体库目录移除、标签合并、transcript 全文修订和播放队列无障碍排序 |
...
Audux 是一个 **纯本地运行的桌面音频播放器与个人音频知识库管理工具**。
...
| 本地 ASR 转写 | 是 | 已实现 |
| transcript 展示 | 是 | 已实现 |
| transcript 全文修订 | 是 | 已实现 |
| transcript 分段保真修订 | 是 | 已实现 |
| transcript 搜索相邻上下文 | 是 | 已实现 |
| transcript 导出 | 是 | 已实现 |
| 本地 LLM 配置 | 是 | 已实现 |
| LLM 隐私警告 | 是 | 已实现 |
...
{
  "full_text": "手动修订后的全文",
  "expected_updated_at": "2026-08-08T12:00:00.000000"
}
```

这是高级替换操作：保存手动全文时清除旧时间轴分段，避免时间戳与文字不一致，并同步更新全文搜索索引。转写任务进行中时拒绝编辑；`expected_updated_at` 与当前版本不一致时返回 `409`。
...
```http
PATCH /audio-items/{audio_id}/transcript/segments
```
...
服务端只修改指定分段的文字，不接受时间戳或顺序变更；保存时按分段顺序重建 `full_text`，并在同一事务中更新 FTS 索引。版本不一致时返回 `409`，避免覆盖较新的 ASR 或用户修订结果。
...
POST /audio-items/batch/organize
```
...
批量整理一次只执行一个显式操作，最多接受 500 个音频 ID：
...
`action` 支持 `add_tags`、`remove_tags`、`add_to_playlist` 和 `set_favorite`。
响应分别统计匹配、修改、未变化、重复 ID 和逐项错误；合法条目的关联写入在同一事务中
提交。批量删除不属于该接口范围。
...
- Transcript revision、来源追踪、质量 issue 和可播放证据锚点
- Segment 级混合检索，以及限定资料库范围的只读 Agent
- 可暂停、可恢复的“转写—验证—勘误—丰富—回写—重建索引”整理 Run
- 有 Transcript 证据的 Tag、描述和章节建议
- 展示精确 diff 后才可执行的低风险资料库工具
- 与应用内 Agent 共用 Tool Registry 的外部只读 MCP 接入
- 说话人分离、词级时间轴和本地术语表等候选增强

当前 P0 是 Agent-ready 的可信内容基础。R1 手动数据库备份与安全恢复、R2 保存视图、
R3 智能 Playlist，以及 R4 资料库健康中心与安全重新关联已经完成。v0.x 保持内部 Beta，
不创建公开 Release，首次公开发布统一为 v1.0。
...
- 显式多选与批量整理、Transcript 保真修订、播放队列与会话连续性已经完成。
- 严格 schema、本地 API、可选 Whisper companion 和三平台构建继续作为内部 Beta 基线。
...
- 建立 metadata、Tag 与 Transcript Segment 的统一检索服务，并可选增加本地混合检索。
- Agent 只能调用后端受控的只读工具，检索范围由后端强制解析。
- 回答引用明确的 Transcript revision、Segment 和时间戳，并可跳转播放。

---

## v0.8：转写、验证、Tag 与勘误闭环

- 整理 Run 串联转写、确定性验证、疑点复核、人工勘误、Tag / 描述 / 章节建议和索引回写。
- 原始 ASR、已接受内容、Agent 提案和可重建索引分层保存。
- 正式勘误和资料库写入必须展示精确 diff，并经用户确认后形成新 revision。

---

## v0.9：安全操作 Agent、MCP 与发布硬化

- 在只读与提案能力稳定后，开放需审批的低风险资料库写工具。
- 基于同一 Tool Registry 提供范围受限的外部只读 MCP 接入。
- 完成归档、诊断、长期运行、安全审计与三平台恢复验证。

---

## v1.0：稳定的 Agent-native 本地音频知识库

- 对检索、引用、转写、勘误、Tag 输出、隐私和回滚作出稳定兼容承诺。
- Agent、LLM 或 embedding 不可用时，播放、手工整理、关键词搜索和导出仍然可用。
...
> 一个纯本地、隐私优先、可播放、可整理，并以有证据 Agent 串联检索、转写、验证与勘误的个人音频知识库。

当前 v0.5 PRD 相比初始 MVP，已经从“基础播放器 + 音频库”扩展为较完整的本地音频知识管理工具。

当前阶段应继续坚持：

1. 本地数据优先。
2. 不默认上传任何用户内容。
3. Agent 的范围与权限由后端强制执行，不依赖 Prompt 自律。
4. AI 只形成可检查的建议，不静默覆盖用户已接受内容。
5. 原始来源、已接受内容、Agent 提案和可重建索引分层保存。
6. 播放、手工编辑、关键词搜索和导出不依赖 Agent 可用性。
7. 所有重要数据可导出、可备份、可迁移。
